...
def load_1000_genomes(pc_cols):
    ...
    kgp_merged = (
        kgp_pop_df
        .merge(kgp_super_pop_df, left_on='Population', right_on='Population Code', how='inner')
        .merge(kgp_pc_df, left_on='Individual ID', right_on='IID', how='inner')
        .filter(items=['IID', 'Population', 'Super Population', 'Gender', *pc_cols])
    )
    return kgp_merged
...
def classify_ukbb(classifier, pc_cols, output_file='data/ukb_filtered/population_labels.tsv.gz', 
                  min_prob=0.9):
    # Load UK Biobank PCA data and store as two arrays (to save memory)
    ukb_pc_df = pd.read_csv(
        'data/ukb_filtered/projection.sscore', 
        sep='\t', 
        usecols=['#FID', 'IID', *pc_cols]
    )
    ...
    ukb_labels_df.to_csv(output_file, sep='\t', compression='infer', index=False)
...
if __name__ == '__main__':
    ...
    del kgp_df
    classify_ukbb(classifier, use_pcs, output_file='data/ukb_filtered/population_labels_10PCS.tsv.gz')
