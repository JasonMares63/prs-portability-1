...
def make_population_sample_files(all_populations_df):
    ...
    for population in ['AFR', 'AMR', 'EAS', 'EUR', 'SAS']:
        (
            ...
            .loc[:, ['#FID', 'IID']]
            .to_csv(f'data/ukb_populations/{population}_all.txt', header=True, index=False, sep=' ')
        )
...
def get_trait_to_n_samples():
    ...
    """
    martin_gwas_info = pd.read_csv('data/martin_gwas_info.txt', sep=' ')
...
def make_trait_sample_files(all_samples_df, trait_to_n_samples, seed=100):
    """
    Sample the appropriate number of 
    """
    for trait, n_samples in trait_to_n_samples.items():
        (
            all_samples_df
            .sample(n=n_samples, random_state=seed)
            .to_csv(f'data/ukb_populations/{trait}.txt', header=True, index=False, sep=' ')
        )
...
if __name__ == '__main__':
    ...
    labels_df = (
        pd.read_csv('data/ukb_filtered/population_labels_10PCS.tsv.gz', sep='\t')
        # .query('inconclusive == False')
        .drop_duplicates(subset=['#FID', 'IID'])
    )

    # 5000 Samples used as the "Target" for EUR in 
    eur_test_df = (
        labels_df
        .query('predicted == "EUR"')
        .sample(n=5000, replace=False, random_state=seed)
    )
    eur_test_df.loc[:, ['#FID', 'IID']].to_csv('data/ukb_populations/EUR_test.txt', index=False, header=True, sep=' ')
