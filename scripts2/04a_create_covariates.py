...
if __name__ == '__main__':
    sex_df = pd.read_csv('/rigel/mfplab/projects/ukb_hakhamanesh/phenotype_data/extracted_phenotypes/ukb.sex.txt', sep=r'\s+',
                         header=None, names=['IID', 'is_male'])

    age_df = pd.read_csv('/rigel/mfplab/projects/ukb_hakhamanesh/phenotype_data/extracted_phenotypes/ukb.age.txt', sep=r'\s+',
    ...
    (
        ...
        .filter(items=['#FID', 'IID', 'sex_covar', 'age', 'age_sq', 'age_sex',
                       'age_sq_sex', *['PC{}_AVG'.format(i) for i in range(1, 21)]])
