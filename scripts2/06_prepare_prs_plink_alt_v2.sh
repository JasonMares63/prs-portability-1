...
do
  ...
  # Update current UK Biobank data to take into account duplicates
  $plink2\
    --memory 35000 \
...
do
  ...
  do
   ...
   #   --output data/gwas_results/${phenotype}.chr${chromosome}.${phenotype}.glm.assoc
    ...
    $plink\
      ...
      --clump /rigel/mfplab/projects/prs-portability/data/gwas_results/${phenotype}.chr${chromosome}.${phenotype}.glm.assoc \
      --clump-p1 0.01 \
      --clump-p2 1 \
      --clump-r2 0.5 \
      --clump-kb 250
  ...
  python scripts2/06d_combine_glm_threshold_4.py \
    /rigel/mfplab/projects/prs-portability/data/gwas_results/${phenotype}.chr*.${phenotype}.glm.linear \
