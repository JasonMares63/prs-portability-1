...
for (pheno in phenotypes) {
  ...
  results_name <- paste0("data/LDpred/",pheno,"_ldpred_auto_results.tsv")
  results <- read_tsv(results_name)

  old_results_name <- paste0("data/LDpred/",pheno,"_merged_sumstats_ldpred.tsv")
  ...
  new_results_name <- paste0("data/LDpred/prs/",pheno,"_LDpred_sumstats.tsv")
